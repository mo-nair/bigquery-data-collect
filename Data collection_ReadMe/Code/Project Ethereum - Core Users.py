...
def rolling_sum_by_days(df, repoName, actorId, column, days):
    ...
    for index, row in df.iterrows():
        ...
        if (actorId != None):
            mask = (df['date1'] >= start_date) & (df['date1'] <= end_date) & (df['GhRepo'] == row[repoName]) & (df['actor_id'] == row[actorId])
        result.append(df.loc[mask, column].sum())
    return result

columns_to_display = ['GhRepo', 'date1', 'actor_id', 'actor_login', 'num_activities', 'user_rolling_sum_30_d', 'repo_rolling_sum_30_d', 'percent_activities_use', 'is_core_developer', 'is_user_bot']

df = pd.read_csv('Data2015.csv', low_memory = False)
pd.set_option('display.max_columns', 30)
df['date1'] = pd.to_datetime(df['date1'], format = '%Y-%m-%d')
df.sort_values(by = ['GhRepo', 'date1', 'actor_id'])
...
user_activities_df = sub_df.groupby(['GhRepo', 'date1', 'actor_id'])
sub_df['user_rolling_sum_30_d'] = rolling_sum_by_days(sub_df.reset_index(), 'GhRepo', 'actor_id', 'num_activities', 30)
...
merged_df[columns_to_display].to_csv('Data2015CoreUser.csv', index = False)